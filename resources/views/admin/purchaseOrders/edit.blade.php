...
@section('description', 'Editar orden o cotización')
                    ...
                    <div class="col-md-3 col-xs-12">
                        <div class="form-group {{ $errors->has('expires') ? 'has-error' : '' }}">
                            ...
                            <div class="input-group date">
                                ...
                                <input value="{{ old('expires', $purchaseOrder->expires) }}" name="expires" type="text" class="form-control pull-right datepicker">
                            </div>
                    ...
                    <div class="col-xs-12 col-md-3">
                        <div class="form-group">
                            ...
                            <select name="type" id="type" style="width: 100%" class="form-control select2" required>
                                <option value="purchaseOrder" {{ old('type', $purchaseOrder->type) === 'purchaseOrder'  ? 'selected' : '' }}>Orden de compra</option>
                                <option value="invoice" {{ old('type', $purchaseOrder->type) === 'invoice'  ? 'selected' : '' }}>Factura</option>
                                <option value="quotation" {{ old('type', $purchaseOrder->type) === 'quotation'  ? 'selected' : '' }}>Cotización</option>
                            </select>
                        </div>
                    </div>
                    <div class="col-md-3 col-xs-12">
...
@push('scripts')
    ...
    <script>
        $(function () {
            ...
            let client_id = '{{ old('client_id', $purchaseOrder->client_id) }}';

            @if(old('products', $purchaseOrder->details))
                @foreach(old('products', $purchaseOrder->details) as $product)
                    products.push({
                        product_id: parseInt('{{ $product['product_id'] }}'),
                        name: "{{ \App\Product::where('id', $product['product_id'])->first()->name }}",
                        row: ""+productId,
                        quantity: parseInt('{{ $product['quantity'] }}'),
                        taxPercentage: parseInt('{{ $product['iva'] ? $product['iva'] : $product['tax_percentage'] }}'),
                        netValue: parseInt('{{ $product['unit_value'] }}'),
                        totalValue: parseInt('{{ $product['total_value'] }}'),
                        unitValue: parseInt('{{ $product['net_value'] ? $product['net_value'] : $product['value'] }}'),
                    });
            ...
            //Code
            if(client_id){
                $.get('{{ route('api.clients.show', old('client_id', $purchaseOrder->client_id)) }}', function(client) {
                    $('#client_id').append(`<option value="{{ old('client_id', $purchaseOrder->client_id) }}" selected>${client.text}</option>`);
                    setFields(client);
                });
            ...
            function setFields(client) {
                ...
                let oldPet = parseInt({{ old('pet_id', $purchaseOrder->pet_id) }});
