...
@section('description', 'Crear mascota')
...
@section('content')
    <div class="row">
        <div class="col-xs-12">
            <form action="{{ route('clients.pets.store', $client) }}" method="POST">
                {{ csrf_field() }}
                    ...
                    <div class="box-body">
                        <div class="row">
                            ...
                            <div class="col-xs-3">
                                <div class="form-group {{ $errors->has('weight') ? 'has-error' : '' }}">
                                    ...
                                    <input  step="any" value="{{ old('weight') }}" type="number" class="form-control" name="weight" id="weight" placeholder="Peso (Kg)">
                            ...
                            <div class="col-xs-3">
                                <div class="form-group {{ $errors->has('gender') ? 'has-error' : '' }}">
                                    ...
                                    <select name="gender" id="gender" class="form-control select2" style="width: 100%;">
                                        ...
                                        <option value="M" @if (old('gender') == "M") {{ 'selected' }} @endif>Macho</option>
                                        <option value="F" @if (old('gender') == "F") {{ 'selected' }} @endif>Hembra</option>
                                    </select>
                                ...
                                <div class="form-group {{ $errors->has('species_id') ? 'has-error' : '' }}">
                                    ...
                                    <select name="species_id" id="species_id" class="form-control select2" style="width: 100%;">
                                        ...
                                        @foreach($species as $kind)
                                            <option value="{{ $kind->id }}" {{ old('species_id') == $kind->id ? 'selected' : '' }}>{{ $kind->name }}</option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="col-xs-4">
                                <div class="form-group {{ $errors->has('reproductive_status') ? 'has-error' : '' }}">
                                    ...
                                    <select name="reproductive_status" id="reproductive_status" class="form-control select2" style="width: 100%;">
                                        ...
                                        <option value="REPRODUCTIVO" @if (old('reproductive_status') == "REPRODUCTIVO") {{ 'selected' }} @endif>Reproductivo</option>
                                        <option value="CASTRADO" @if (old('reproductive_status') == "CASTRADO") {{ 'selected' }} @endif>Castrado</option>
                                    </select>
                        ...
                        <div class="col-xs-12">
                            <div class="form-group">
                                <button type="submit" class="btn btn-primary btn-block">Guardar mascota</button>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <script>
        $(function () {
            ...
            function getBreeds(species_id) {
                ...
                let oldBreed = parseInt({{ old('breed_id') }});
                $.get(url, (response)=>{
                    $('#breed_id').empty();
