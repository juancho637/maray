...
<td>
    @if(Carbon\Carbon::parse($engagement->date)->format('d-m-Y') >= Carbon\Carbon::now()->format('d-m-Y'))
        @if($engagement->state->abbreviation === 'appoint-atten')
            ...
            @if($engagement->purchaseOrder->type === null)
                <a href="{{ route('purchaseOrders.edit', $engagement->purchaseOrder->id) }}" class="btn btn-xs btn-success" style="padding: 1px 7px;"><i class="fa fa-usd" aria-hidden="true"></i></a>
            @endif
        @endif
