...
<main>
    ...
    @endif
    @if(count($balance->credits()->creditInvoice()->get()) > 0)
        <table class="table table-striped">
            <thead>
            <tr>
            ...
            <tbody>
            @foreach($balance->credits()->creditInvoice()->get() as $credit)
                <tr>
                    <td>{{ $credit->id }}</td>
    ...
    @endif
    @if(count($balance->creditPayments()->creditInvoice()->get()) > 0)
        <table class="table table-striped">
            <thead>
            <tr>
            ...
            <tbody>
            @foreach($balance->creditPayments()->creditInvoice()->get() as $creditPayment)
                <tr>
    ...
    <table class="table table-striped">
        ...
        <tr>
            <td>Efectivo</td>
            <td colspan="2">${{ number_format($balance->manual_invoice_cash) }}</td>
            <td colspan="2">${{ number_format($balance->system_invoice_cash) }}</td>
        </tr>
        <tr>
            <td>Cheque</td>
            <td colspan="2">${{ number_format($balance->manual_invoice_cheque) }}</td>
            <td colspan="2">${{ number_format($balance->system_invoice_cheque) }}</td>
        </tr>
        <tr>
            <td>T. débito / T. crédito</td>
            <td colspan="2">${{ number_format($balance->manual_invoice_card) }}</td>
            <td colspan="2">${{ number_format($balance->system_invoice_card) }}</td>
        </tr>
        <tr class="warning">
            <td>Total</td>
            <td colspan="2">${{ number_format($balance->manual_invoice_total) }}</td>
            <td colspan="2">${{ number_format($balance->system_invoice_total) }}</td>
        </tr>
        </tbody>
    </table>
    <br>
